...
# Tests the httpbin TLS egress demo happy path:
...
# Check EgressGateway
GW_STATUS=$(kubectl --context "${CONTEXT}" get egressgateway egress-tls -o jsonpath='{.status.conditions[?(@.type=="Programmed")].status}' 2>/dev/null || true)
if [ "${GW_STATUS}" = "True" ]; then
  pass "EgressGateway egress-tls is Programmed"
else
  fail "EgressGateway egress-tls is not Programmed (status: ${GW_STATUS:-unknown})"
fi

# Check HTTPRoute
HR_STATUS=$(kubectl --context "${CONTEXT}" get httproute httpbin-egress-tls-route -o jsonpath='{.status.parents[0].conditions[?(@.type=="Accepted")].status}' 2>/dev/null || true)
if [ "${HR_STATUS}" = "True" ]; then
  pass "HTTPRoute httpbin-egress-tls-route is Accepted"
else
  fail "HTTPRoute httpbin-egress-tls-route is not Accepted (status: ${HR_STATUS:-unknown})"
fi

# Check envoy proxy pod
ENVOY_READY=$(kubectl --context "${CONTEXT}" get pods -l app=envoy-egress-tls -o jsonpath='{.items[0].status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)
...
# Start port-forward
kubectl --context "${CONTEXT}" port-forward svc/envoy-egress-tls "${LOCAL_PORT}:${REMOTE_PORT}" &>/dev/null &
PF_PID=$!
sleep 2
