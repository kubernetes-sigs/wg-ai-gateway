...
# Sets up the httpbin egress demo environment:
#   Kind cluster, egress controller, EgressGateway, Backend, and HTTPRoute.
#
...
echo "==> Deploying example EgressGateway, Backend, and HTTPRoute..."
make example
...
kubectl --context "${CONTEXT}" wait --for=condition=ready pod -l app.kubernetes.io/component=proxy --timeout=120s 2>/dev/null || \
  kubectl --context "${CONTEXT}" wait --for=condition=ready pod -l app=envoy-egress --timeout=120s 2>/dev/null || \
...
for i in $(seq 1 30); do
  STATUS=$(kubectl --context "${CONTEXT}" get egressgateway egress -o jsonpath='{.status.conditions[?(@.type=="Programmed")].status}' 2>/dev/null || true)
  if [ "${STATUS}" = "True" ]; then
    break
  fi
  sleep 2
