...
import (
	...
	"sigs.k8s.io/wg-ai-gateway/prototypes/backend-control-plane/pkg/schema/gvk"
)
...
func isAllowedByListener(gateway *gatewayv1.Gateway, listener gatewayv1.Listener, route metav1.Object, namespaceLister corev1listers.NamespaceLister) bool {
	...
	if allowed == nil {
		...
		return routeNamespace == gateway.GetNamespace()
	}
	...
	var routeGVK gvk.GroupVersionKind
	switch route.(type) {
	case *gatewayv1.HTTPRoute:
		routeGVK = gvk.HTTPRoute
	case *gatewayv1.GRPCRoute:
		routeGVK = gvk.GRPCRoute
	default:
	...
	for _, allowedKind := range allowed.Kinds {
		...
		if routeGVK.Kind == string(allowedKind.Kind) && routeGVK.Group == string(allowedGroup) {
			return true
		}
	}
