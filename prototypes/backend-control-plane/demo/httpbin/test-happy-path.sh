...
# Tests the httpbin demo happy path:
...
# Check Gateway
GW_STATUS=$(kubectl --context "${CONTEXT}" get gateway httpbin-gateway -o jsonpath='{.status.conditions[?(@.type=="Programmed")].status}' 2>/dev/null || true)
if [ "${GW_STATUS}" = "True" ]; then
  pass "Gateway httpbin-gateway is Programmed"
else
  fail "Gateway httpbin-gateway is not Programmed (status: ${GW_STATUS:-unknown})"
fi

# Check HTTPRoute
HR_STATUS=$(kubectl --context "${CONTEXT}" get httproute httpbin-route -o jsonpath='{.status.parents[0].conditions[?(@.type=="Accepted")].status}' 2>/dev/null || true)
if [ "${HR_STATUS}" = "True" ]; then
  pass "HTTPRoute httpbin-route is Accepted"
else
  fail "HTTPRoute httpbin-route is not Accepted (status: ${HR_STATUS:-unknown})"
fi

# Check envoy proxy pod
ENVOY_READY=$(kubectl --context "${CONTEXT}" get pods -l app=envoy-httpbin-gateway -o jsonpath='{.items[0].status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)
...
# Start port-forward
kubectl --context "${CONTEXT}" port-forward svc/envoy-httpbin-gateway "${LOCAL_PORT}:${REMOTE_PORT}" &>/dev/null &
PF_PID=$!
sleep 2
